...
счета, затем объединяет истории всех счетов, фильтрует их по `isIncome` и
сортирует по `transactionDate` по убыванию. Domain `Expense` и `Income` хранят
точный `occurredAt`; `occurredOn` выводится из него для сценариев, где нужна
только календарная дата.
