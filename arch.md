...
- наблюдает `TransactionsRepository` и `AccountsRepository`: история и счета
  приходят из Room через domain-контракты, а сетевой refresh только обновляет
  локальное хранилище;
...
- формирует presentation-подзаголовок из имени счёта; если связанный локальный
  счёт ещё не найден, показывает устойчивый fallback «Неизвестный счёт»;
...
Любой HTTP-запрос контура синхронизации, получивший 5xx, выполняется максимум
три раза с интервалом две секунды: это относится и к отправке outbox, и к
загрузке категорий, счетов и истории транзакций. После третьего ответа 5xx
ошибка передаётся вызывающему слою; WorkManager может позднее повторить уже весь
цикл синхронизации. Ошибки 400/401/404 не ретраятся автоматически и для
outbox-запросов сохраняются как постоянная ошибка конкретной pending-операции.
